...
class StatusInStock extends Component {
  constructor() {
    super();
    this.state = {};
  }
  ...
  orderDetail(event) {
    ...
    api.get(`/api/admin/order/detail/${orderNumber}`).then(res => {
      for (let n in res.data) {
        ...
        $("#order-date-detail").html(
          "orderDate : " + res.data[n].orderDate.split("T1")[0]
        );
        $("#required-date-detail").html(
          "orderDate : " + res.data[n].requiredDate.split("T1")[0]
        );
        if (res.data[n].shippedDate != null)
          $("#shipped-date-detail").html(
            "shippedDate : " + res.data[n].shippedDate.split("T1")[0]
          );
      ...
      $("#status-detail").empty();
      $("#status-detail").append(
        "status :" +
          '<select id="order-' +
          res.data[0].orderNumber +
          '" class="status-selected status-detail-selected-input" id="sel1" name="' +
          res.data[0].orderNumber +
          '">' +
          '<option value="Cancelled">Cancelled</option>' +
          '<option value="Disputed">Disputed</option>' +
      ...
      if (res.data[0].status == "Cancelled")
        $(`.status-detail-selected-input`).val("Cancelled");
      else if (res.data[0].status == "Disputed")
        $(`.status-detail-selected-input`).val("Disputed");
      else if (res.data[0].status == "In Process")
        $(`.status-detail-selected-input`).val("In Process");
      else if (res.data[0].status == "On Hold")
        $(`.status-detail-selected-input`).val("On Hold");
      else if (res.data[0].status == "Resolved")
        $(`.status-detail-selected-input`).val("Resolved");
      ...
      $("#customer-number-head").html(
        "Customer number : " + res.data[0].customerNumber
      );
    });
  }
...
class StatusPreOrder extends Component {
  constructor() {
    super();
    this.state = {};
  }
  ...
  editstatus(event) {
    ...
    if (comment.length != 0) {
      console.log("not empty");
      api.get(`/api/admin/preorder/editdetail/${comment}/${orderno}`);
    }
    window.location.reload(true);
  }
  ...
  changepage(event) {
    ...
    var init = current.value * 15;
    var i;
    api.get(`/api/admin/preorder/changepage/status/${init}`).then(res => {
      console.log(res);
      $("#order-data").empty();
      for (i = 0; i < res.data.length; i++) {
        var shippedDate, requiredDate, orderDate;
  ...
  update(event) {
    ...
    console.log(value);
    if (value == "Shipped") {
      api.get(`/api/admin/preorder/setShippedDate/${orderNumber}`).then(res => {});
    }

    api.get(`/api/admin/preorder/update/${value}/${orderNumber}`).then(res => {});
    window.location.reload(true);
  }
  ...
  orderDetail(event) {
    ...
    var total = 0;
    var comment;
    api.get(`/api/admin/preorder/detail/${orderNumber}`).then(res => {
      for (let n in res.data) {
        console.log(res.data[n].quantityOrdered);
        comment = res.data[n].comments;
        if (comment == null) {
      ...
      if (res.data[0].status == "Cancelled")
        ...
        $(`.status-detail-selected-input`).val("Resolved");
      else $(`.status-detail-selected-input`).val("Shipped");
      $("#status-comment-input").val(comment);
      $("#order-number-head").html(res.data[0].orderNumber);
      $("#customer-number-head").html(
        "Customer number : " + res.data[0].customerNumber
      ...
      <div id="wrapper">
        ...
        <div id="content-wrapper" className="d-flex flex-column">
          <div id="content">
            ...
            <div className="container-fluid">
              <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800 sfmono">
                  Pre order status{" "}
                </h1>
              </div>
              <div
...
export { StatusInStock, StatusPreOrder };
